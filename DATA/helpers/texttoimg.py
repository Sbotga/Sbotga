...
def texttoimg(text, width, savefilename):
    ...
    img_1 = img_1.crop(
        (0, 0, width, font.getsize(str(text))[1] * (text.count("\n") + 1) + 40)
    )
...
def t2i(
    ...
    for line in lines:
        ...
        draw.text(
            (-offset_x + padding[2], -offset_y + padding[0]), line, font_color, font
        )
...
def union(
    ...
    if not img_ls:
        ...
        default_image = Image.new(
            "RGBA", (default_width, default_height), bk_color or (0, 0, 0, 0)
        )
    ...
    pic = Image.new(
        "RGBA", (width + padding[2] + padding[3], height + padding[0] + padding[1])
    )
    draw = ImageDraw.Draw(pic)
    if border_type == "circle":
        r = border_radius if border_radius else int(min(pic.width, pic.height) / 36)
        draw.rounded_rectangle(
            (0, 0, pic.width, pic.height), r, bk_color, border_color, border_size
        )
    elif border_type == "rectangle":
        draw.rectangle(
            (0, 0, pic.width, pic.height), bk_color, border_color, border_size
        )
    else:
    ...
    for idx, img in enumerate(img_ls):
        ...
        if attr == "height":
            ...
            if interval_size > 0 and idx != len(img_ls) - 1:
                draw = ImageDraw.Draw(pic)
                pos = (
                    _w + padding[2] - space // 2,
                    padding[0],
                    _w + padding[2] - space // 2,
                    pic.height - padding[1],
                )
